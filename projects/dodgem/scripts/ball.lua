...
Ball = Entity("Ball",{
	...
	spawn=function(self)
		...
		-- beach: start in left side
	end,
	is_colliding = function(self, other)
		return Math.distance(other.x, other.y, self.x, self.y) <= self.radius
	end,
	update = function(self, dt)
		-- smile/soccer: bounce off bottom of screen
		if (self.anim_frame == SMILE or self.anim_frame == SOCCER) and (self.y + self.radius > Game.height or self.y - self.radius < 0) then 
			self.vspeed = -self.vspeed
		end
		-- ball: move in sine wave
		if self.anim_frame == BEACH then 
			self.y = Math.sinusoidal(self.height, Game.height - self.height)
		end
