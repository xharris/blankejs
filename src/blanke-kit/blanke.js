...
const isFloat = n =>
  typeof n == "string" ? n.includes(".") : Number(n) === n && n % 1 !== 0
...
// TODO: doesn't work?
const watchObject = (obj, fn) => {
  const watcher = {
    get(obj, k) {
      if (typeof obj[k] === 'object' && obj[k] !== null) {
        return new Proxy(obj[k], watcher)
      } else {
        return obj[k]
      }
    },
...
class BlankeListView {
  constructor(options) {
    ...
    let controls = {
      add: [
        ...
        function (e) {
          ...
          if (ret !== false) this_ref.addItem(ret || new_item)
        },
      ],
      "move-up": [
        "chevron-up",
        `move ${this.options.object_type} up`,
        e => {
          if (this.el_selected) {
      ...
      "move-down": [
        "chevron-down",
        `move ${this.options.object_type} down`,
        e => {
          if (this.el_selected) {
    ...
    for (let ctrl of this.options.controls) {
      let el_action = blanke.createElement("button", [
        "ui-button-sphere",
        ctrl,
      ])
      let el_icon = blanke.createElement("i", [
        "mdi",
        "mdi-" + controls[ctrl][0],
      ])
  ...
  addItem(text) {
    ...
    if (this.selected_text == text) this.selectItem(text)
  ...
  selectItem(text) {
    ...
    for (let c = 0; c < children.length; c++) {
      ...
      if (children[c].el_text.innerHTML == text) el_selected = children[c]
    }
...
class BlankeForm {
  ...
  constructor(inputs, dark) {
    ...
    if (dark) this.container.classList.add("dark")
  }
  ...
  addInput(input, internal) {
    ...
    if (input.length <= 2 && typeof input_type != "string") {
      ...
      if (!this.first_header) this.first_header = el_header
    ...
    if (input_type == "checkbox") container_type = "label"
    ...
    if (show_label !== false) el_container.appendChild(el_label)
    ...
    if (input_type == "icon-button") {
      let el_button = blanke.createIconButton(
        extra_args.icon || label,
        input_name
      )
      this.prepareInput(el_button, input_name)
      el_inputs_container.appendChild(el_button)
    ...
    if (input_type == "text" || input_type == "number") {
      ...
      for (var i = 0; i < input_count; i++) {
        ...
        if (Array.isArray(extra_args.default)) {
          el_text.value = ifndef(
            extra_args.default[i],
            input_type == "text" ? "" : "0"
          )
        } else
          el_text.value = ifndef(
    ...
    if (input_type == "directory" || input_type == "file") {
      ...
      el_file_btn.addEventListener("click", e => {
        blanke.chooseFile(
          {
            properties: [
              input_type == "directory" ? "openDirectory" : "openFile",
            ],
          },
          file_path => {
            if (file_path.length > 0) el_input.value = file_path
    ...
    if (prepend_inputs) el_container.prepend(el_inputs_container)
    else el_container.appendChild(el_inputs_container)
    el_container.setAttribute("data-name", input_name)

    this.container.appendChild(el_container)
    if (!internal); // this.hideHeader(this.first_header); // TODO what is hideHeader?
  }
  ...
  prepareInput(element, name, index) {
    ...
    // tooltip
    if (args.desc) element.title = args.desc
    // default value
    if (args.default)
      this.input_values[name][index || 0] =
        index != null ? args.default[index] : args.default
    else this.input_values[name][index || 0] = input_type == "text" ? "" : 0
  }
  ...
  onChange(input_name, func) {
    ...
    for (var input of this.input_ref[input_name]) {
      ...
      if (
        ["color", "select", "checkbox"].includes(this.input_types[input_name])
      )
        event_type = "change"
      if (["button", "icon-button"].includes(this.input_types[input_name]))
        event_type = "click"
      ...
      input.addEventListener(event_type, function (e) {
        ...
        if (
          ...
          val = e.target.value
          if (val == "" && e.target.placeholder) val = input_args.default || ""
        }
        ...
        if (input_type == "number")
          val = isFloat(e.target.value)
            ? parseFloat(e.target.value)
            : parseInt(e.target.value)

        input_value[parseInt(e.target.dataset["index"]) || 0] = val
        let ret_val = func(
          input_value.length == 1 ? input_value[0] : input_value.slice()
        )
  ...
  getValue(input_name, index) {
    ...
    else return this.input_ref[input_name][index].value
  }
  ...
  setValue(input_name, value, index) {
    ...
    } else this.input_ref[input_name][index].value = value
...
class Toast {
  constructor(text, duration, die_cb) {
    ...
    this.el_icon.addEventListener("click", () => {
      this.die(0)
    })
    ...
    // duration
    if (duration == null || duration > 0) this.die(duration, die_cb)
  ...
  die(t, cb) {
    setTimeout(() => {
      let animation = this.el_new_toast.animate(
        [{ opacity: 1 }, { opacity: 0 }],
        { duration: 200, iterations: 1, easing: "ease-in" }
      )
      animation.pause()
      animation.onfinish = () => {
  ...
  set style(v) {
    ...
    if (styles[v]) this.el_icon.style.backgroundColor = "#" + styles[v]
  }
}
  ...
  showModal: function (html_body, choices) {
    ...
    choice_keys.forEach(function (c) {
      ...
      blanke.getElement(
        ...
        blanke
          .getElement("body > .ui-modal[data-uuid='" + uuid + "']")
          .remove()
  ...
  createWindow: function (options) {
    ...
    $(el).resizable({
      stop: function (event, ui) {
        if (onResizeStop) onResizeStop(event, ui)
      },
    })
