...
class AssetManager extends Editor {
  ...
  addFile(path) {
    ...
    el_file_row.ondragstart = defineDragData(
      "text/plain",
      (path.match(/[\\\/\w]\/assets\/\w+\/(.*)\./, "") || ["", path])[1]
    );
  ...
  previewFile(path) {
    ...
    let file_folder = nwPATH
      .dirname(app.getRelativePath(path))
      .replace(/assets[/\\]/, "");

    el_file_form.setValue(
      "filename",
      nwPATH.basename(path).replace(file_ext, "")
    );
    ...
    el_file_form.onChange("filename", function (value) {
      blanke.cooldownFn("file_rename", 2000, function () {
        let new_path = nwPATH.resolve(
          nwPATH.join(app.project_path, "assets", file_folder, value + file_ext)
        );
    ...
    el_file_form.onChange("folder", function (value) {
      blanke.cooldownFn(
        ...
        function () {
          ...
          try {
            nwFS.move(old_path, new_path, { overwrite: true }, function (err) {
              // move file
              ...
              file_folder = nwPATH
                .dirname(app.getRelativePath(new_path))
                .replace(/assets[/\\]/, "");
