...
class Console extends Editor {
  ...
  tryClose() {
    if (!this.had_error && !this.pin) this.close();
  }
  ...
  parse(line) {
    ...
    return line.map(l =>
      typeof l == "object" ? nwUTIL.inspect(l, { compact: true }) : l
    );
  }
  ...
  log(...args) {
    ...
    if (this.last_line == str) {
      ...
      this.el_lines.children[
        this.el_lines.children.length - 1
      ].innerHTML = `${str} (${this.duplicate_count})\n`;
    } else {
      this.duplicate_count = 1;
