...
class DragBox {
  ...
  show() {
    this.hidden = false
    this.drag_container.classList.remove("invisible")
  }

  hide() {
    this.hidden = true
    this.drag_container.classList.add("invisible")
  }
  ...
  close() {
    ...
    dragboxes = dragboxes.filter(box => box.guid != this.guid)
  ...
  static closeAll(type) {
    dragboxes.forEach(box => {
      if (!type || type == box.drag_container.dataset.type)
        box.remove()
    })
  }

  static toggleHiddenAll() {
    DragBox.all_hidden = !DragBox.all_hidden
    dragboxes.forEach(box => {
      box.toggleHidden()
    })
  }
